 ...
 * generation on it.
 *
 * v2 calls this once per photo in the tray, several in flight at a time, so
 * the response stays as small as it can: the caller already holds the file
 * and renders its own thumbnail, and the cropped preview the v1 app returned
 * was a base64 JPEG per photo that nothing on screen used.
 */
